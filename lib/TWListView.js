...
class TWListView extends Component{
    ...
    render(){
        ...
        return(
            ...
            renderHeader={() =>{
              if(dataSource.getSectionLengths()  <= 0 && !isLoading && !this.state.isLoadMore){
            ...
            onScroll={event => {
              ...
              if (Platform.OS === "ios") {
                if (scrollY > event.nativeEvent.contentSize.height + 20 && event.nativeEvent.contentOffset.y > 0) {
                  if (!this.state.isLoadMore) {
                    this.setState({
                      isLoadMore: true
                    });
                ...
                if (scrollY > event.nativeEvent.contentSize.height - 50 && event.nativeEvent.contentOffset.y > 0) {
